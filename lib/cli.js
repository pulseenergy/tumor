...
function projectFromFile(file, cb) {
	readJson(file, function (err, json) {
		...
		var project = projects[json.name] = {
			...
			exists: true,
			deps: [],	// TODO: merge with dependencies and filter linked dependencies later
			dependencies: {}
		};
...
steps.deps = [
	eachProject(function (project, cb) {
		...
		if (project.missing) {
			...
			var dependencyToMatch = argv[1] || argv[0];
			_.each(project.dependencies, function (version, name) {
				if (name === dependencyToMatch) {
					output(name + ': ' + version);
				}
			});
			cb();
