...
function checkIfDepInstalled(project) {
	return function (depName, cb) {
		...
		readJson(depPkgFile, function (err, depPkgInfo) {
			if (err) return cb(err);
...
function readProjectDeps() {
	return function (cb) {
		async.each(_.values(projects), function (project, cb) {
			async.each(_.keys(project.dependencies), checkIfDepInstalled(project), cb);
		}, cb);
	};
}
