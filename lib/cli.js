...
function linkDependencies(project, cb) {
	console.log('>>> linking modules for ' + color.name(project.name));
...
function installProject(project, cb) {
	if (project.installed) {
		console.log('>>> npm install for ' + color.name(project.name) + ' skipped because it was the target of a npm link');
		cb();
	} else {
		console.log('>>> npm install for ' + color.name(project.name));
...
function execCommand(project, cb) {
	if (project.exists) {
		console.log('>>> exec for ' + color.name(project.name));
		exec(argv[0], argv.slice(1), project.path, function (err) {
			cb(failfast ? err : null);
		});
	} else {
		cb();
	}
...
steps.status = [
	...
	eachProject(function (project, cb) {
		...
		function output() {
			if (!logged) {
				console.log('>>> ' + color.name(project.name));
				logged = true;
			}
			if (arguments.length) {
				console.log.apply(console, arguments);
