...
function cloneProject(project, cb) {
	...
	var url = project.repository.replace(/^git:\/\//, 'git@').replace(/#.*$/, '').replace("github.com/", "github.com:");
	console.log("URL", url);
			...
			if (project.exists || project.missing) {
				...
				projectFromFile(file, function (err, result) {
					if (err) {
						...
						readProjectDeps(false)(project, function (err) {
							if (err) return cb(err);
							cb(null, result);
						});
					}
