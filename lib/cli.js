...
function shouldLink(name, version, repoUrl) {
	if (conf.link.override[name] !== undefined) {
		return String(conf.link.override[name]) !== 'false';
	}
	return _.any(matchers, function (matcher) {
		if (version && version.indexOf(matcher) !== -1) {
			return true;
		}
		if (repoUrl && repoUrl.indexOf(matcher) !== -1) {
			return true;
		}
		return false;
	});
...
function readDep(project) {
	return function (depName, cb) {
		...
		readJson(depFilePath, function (err, depPkgInfo) {
			if (err) {
			...
			if (!depRepoUrl){
				verbose(">>> dep " + depName + " of " + project.name + " has no repository specified in package.json");
			}

			if (shouldLink(name, version, depRepoUrl)) {
...
function cloneProject(project, cb) {
	...
	var url = project.repository.replace(/^git:\/\//, 'git@').replace(/#.*$/, '');
	if (/^git/.test(url)) {
		url = url.replace("github.com/", "github.com:");
	}
	verbose('cloning ' + url + ' into ' + project.path);
...
steps.json = [
	...
	function (cb) {
		console.log(JSON.stringify(_.map(projects, function (project) {
			return _.pick(project, 'name', 'path', 'repository', 'deps', 'missing');
